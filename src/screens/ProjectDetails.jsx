...
import { FaBed, FaBath, FaPhone, FaWhatsapp, FaArrowLeft, FaArrowRight, FaPlay, FaPause, FaMapMarkerAlt, FaHome, FaBuilding, FaClock, FaExpand, FaChevronDown, FaChevronUp } from "react-icons/fa";
...
function ProjectDetails() {
  ...
  // Get the return state that was passed from Projects component
  ...
  // Determine area label based on project category
  const areaLabel = project?.category?.toLowerCase() === "apartment" || 
  ...
  useEffect(() => {
    ...
    const apiUrl = import.meta.env.VITE_BACKEND_URL + "/api/projects/" + id;
    ...
    fetch(apiUrl)
      ...
      .then((data) => {
        setProject(data);
        const defaultMessage = "I would like to inquire about your project " + data.project_name + " - ID (" + data.id + "). Please contact me at your earliest convenience.";
        setMessage(defaultMessage);
        setLoading(false);
      })
      .catch((err) => {
        setErrorMessage("Could not load project details: " + err.message);
        setLoading(false);
      });
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    ...
    setEmail("");
    setPhone("");
    const resetMessage = "I would like to inquire about your project " + project.project_name + " - ID (" + project.id + "). Please contact me at your earliest convenience.";
  ...
  if (!project) return <div className="flex items-center justify-center min-h-screen px-4 text-center brockman-font">Project not found</div>;
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Header />
      <main className="flex-1 bg-gray-50 py-4 md:py-10 px-3 md:px-6 lg:px-12 mt-16 md:mt-20">
        ...
        <div className="mb-4 md:mb-6">
          ...
          >
            ...
            <span className="brockman-font text-sm md:text-base">Back to Projects</span>
          </button>
        </div>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="mt-4 md:mt-6">
              <div className="flex flex-wrap items-center gap-2 mb-3">
                <span className={`text-xs px-2 py-1 rounded-full font-semibold brockman-font ${getStatusColor(project.status)}`}>
                  {project.status?.toUpperCase() || 'COMPLETED'}
                </span>
                <span className="text-xs bg-red-600 text-white px-2 py-1 rounded-full font-semibold brockman-font">
                  FOR {project.project_type?.toUpperCase()}
                </span>
                <span className="text-xs bg-purple-100 text-purple-600 px-2 py-1 rounded-full font-semibold flex items-center gap-1 brockman-font">
                  {getCategoryIcon(project.category)}
                  {project.category}
                </span>
              </div>
              <h1 className="text-xl md:text-2xl font-bold text-gray-900 mb-2 brockman-font">{project.project_name}</h1>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6 text-gray-700">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between border-b py-2 text-sm md:text-base">
                      <span className="font-medium brockman-font">Category</span>
                      <span className="text-right brockman-font">{project.category}</span>
                    </div>
                    <div className="flex justify-between border-b py-2 text-sm md:text-base">
                      <span className="font-medium brockman-font">Price</span>
                      <span className="brockman-font"><span className="text-xs md:text-sm mr-1">PKR</span>{project.price}</span>
                    </div>
                    <div className="flex justify-between border-b py-2 text-sm md:text-base">
                      <span className="font-medium brockman-font">Location</span>
                      <span className="text-right brockman-font">{project.location}</span>
                    </div>
                    <div className="flex justify-between border-b py-2 text-sm md:text-base">
                      <span className="font-medium brockman-font">Area</span>
                      <span className="flex items-center gap-1">
                      <span className="brockman-font">{Number(project.area_size).toLocaleString()} {areaLabel}</span>
                      </span>
                    </div>
                    ...
                    <div className="md:hidden flex justify-between border-b py-2 text-sm">
                      ...
                      <span className="brockman-font">{project.project_type}</span>
                    </div>
                    <div className="md:hidden flex justify-between border-b py-2 text-sm">
                      <span className="font-medium brockman-font">Bedroom(s)</span>
                      <span className="brockman-font">{formatValue(project.beds)}</span>
                    </div>
                    <div className="md:hidden flex justify-between border-b py-2 text-sm">
                      <span className="font-medium brockman-font">Bath(s)</span>
                      <span className="brockman-font">{formatValue(project.baths)}</span>
                    </div>
                    {project.area_size && (
                      <div className="md:hidden flex justify-between border-b py-2 text-sm">
                        <span className="font-medium brockman-font">Completion</span>
                        <span className="brockman-font">{project.completion}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-4 p-3 md:p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex gap-4 text-xs md:text-sm text-gray-600">
                      {project.images?.length > 0 && (
                        <span className="brockman-font">📷 {project.images.length} Photo{project.images.length > 1 ? 's' : ''}</span>
                      )}
                      {project.videos?.length > 0 && (
                        <span className="brockman-font">📹 {project.videos.length} Video{project.videos.length > 1 ? 's' : ''}</span>
                      )}
                    </div>
                  </div>
                )}
                  ...
                  <div className="relative">
                    ...
                    <div 
                      className={`overflow-hidden transition-all duration-700 ease-in-out transform ${
                        isDescriptionExpanded || !shouldTruncateDescription(project.description)
                    ...
                    {shouldTruncateDescription(project.description) && !isDescriptionExpanded && (
                      <div 
                ...
                {project.latitude && project.longitude ? (
                  <div className="relative z-0 rounded-lg overflow-hidden border border-gray-200">
                    ...
                    >
                      ...
                      />
                      <Marker position={[project.latitude, project.longitude]} icon={markerIcon}>
                        <Popup>
                          <span className="brockman-font">{project.project_name} - {project.location}</span>
                        </Popup>
                      </Marker>
                    </MapContainer>
                  </div>
                ) : (
                  <p className="text-sm md:text-base brockman-font">Map not available for this project.</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-1 hidden lg:block">
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-24">
              <p className="text-2xl font-bold text-gray-800 mb-2 brockman-font">
                <span className="text-sm mr-1">PKR</span>{project.price}
              </p>

              {/* Contact Buttons */}
              <div className="flex gap-3 mb-3">
                <a
                  href={"https://wa.me/923008213333?text=" + encodeURIComponent("I would like to inquire about your project " + project.project_name + " - ID (" + project.id + "). Please contact me at your earliest convenience.")}
                  target="_blank"
